...
var activeBeatmapPreviewId=-1;var audio=new Audio();audio.volume=0.45;audio.onended=function(){$('.bmlistt>.icon-pause').removeClass("icon-pause").addClass("icon-play");}
function playBeatmapPreview(id)
{if(activeBeatmapPreviewId==id)
{if(audio.paused)
{audio.play();return true;}
else
{audio.pause();audio.currentTime=0;return false;}}
activeBeatmapPreviewId=id;audio.src=STATIC_DOMAIN_BEATMAP+"/preview/"+ id+".mp3";audio.play()
return true;}
function load(id,e)
{if(e.button!==1)
loadUrl("/s/"+ id);}
...
{$(".login-dropdown").slideDown(100,function(){flashLogin($(".login-dropdown"),10,100);});scrollTo('body');if($("#username-field").val().length==0)
$("#username-field").select();else
$("#password-field").select();return false;}
function showLoginForm()
{if($(this).hasClass("require-login"))
{flashLoginForm();return;}
$(".login-dropdown").slideToggle(100);scrollTo('body');if($("#username-field").val().length==0)
$("#username-field").select();else
$("#password-field").select();return false;}
